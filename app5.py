...
def map5_view():
    ...
    for data in weather_data_list:
        ...
        uv = data['uv']  # Cloud cover percentage
    ...
    grid_uv = griddata((lons, lats), uvs, (lon, lat), method='cubic')
    # Plot the contour map with color fill
    uv_boundaries = [0, 2, 5, 7, 10, 12]  # Add one more boundary at 12
    uv_labels = ["Low", "Moderate", "High", "Very High", "Extreme", "Extreme+"]  # Updated label for higher UV index
    # Define the corresponding colors for each UV range
    colors = [
        '#00FF00',  # Green for "Low"
        '#FFFF00',  # Yellow for "Moderate"
        '#FF8000',  # Orange for "High"
        '#800080',  # Purple for "Very High"
        '#ADD8E6',  # Light Blue for "Extreme"
        '#0000FF'   # Blue for "Extreme+" (can represent higher UV values)
    ]

    # Create a linear segmented colormap with the gray shades
    cmap = mcolors.ListedColormap(colors)
    norm = mcolors.BoundaryNorm(uv_boundaries, cmap.N)
    contour = ax.contourf(lon_grid, lat_grid, grid_uv, levels= uv_boundaries, cmap=cmap, norm=norm, alpha= 0.6, transform=ccrs.PlateCarree())


    # Add colorbar for cloud cover
    cbar = fig.colorbar(contour, ax=ax, orientation="vertical", shrink=0.5, label="UV Index")
    cbar.set_ticks(uv_boundaries)  # Set ticks at the boundaries
    cbar.set_ticklabels(uv_labels)  # Set custom labels for the ticks
    ...
    ax.set_title(f"UV Observations as of {formatted_time}")
